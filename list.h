...
struct Node_s {
    ...
    void *item; //item saved in node
    int prev; //index of its previous node in node_array
    int next; //index of its next node in node_array
    int next_available_node; //index of next free node in node_array that is available to use
};

typedef struct {
    // TODO: You should change this!
    int pos; //position of the list in head_array
    int count;  //numer of nodes in list, 0 if empty list
    int curr; //index of the current node in node_array, -1 if empty list or beyond the list, -2 if beyond the list
    int first; //similar as curr, -1 if list is empty
    int last;  //similar as curr, -1 if list is empty
    int next_available_head; //index of next free list(head) in the list_array that is available to use, -1 if no more head avilable
